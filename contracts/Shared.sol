...
contract Session is Shared {
    ...
    function calcFinalPrice(uint256 _sessionId) public view returns (uint256){
        ...
        // list of bids from 1 participant
        //uint256[] memory bList;
        uint256 bid;
        uint256 sum;
        uint256 finalPrice;
        address pAddress;
        for(uint256 i =0; i< pList.length;i++){
            ...
            bid = bidAverage(_sessionId, pAddress);
            sum += bid;
        }
    ...
    function bidAverage(uint _sessionId, address _participant) public view returns (uint256){
